"use client"

import { useState, useMemo } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { Country } from "@/types/Country";
import CountryCard from "./CountryCard";

type Props = {
    country: Country[];
    initialRegion: string;
};

const REGIONS = ["Africa", "America", "Asia", "Europe", "Oceania"];

export default function CountriesClient({ country, initialRegion }: Props) {
    const router = useRouter();
    const [search, setSearch] = useState("");
    const [activeRegion, setActiveRegion] = useState(initialRegion);

    // filter countries based on search and region
    const filtered = useMemo(() => {
        return country.filter((c) => {
            const matchesSearch = c.name.common.toLowerCase().includes(search.toLocaleLowerCase());
            const matchesRegion = activeRegion ? c.region === activeRegion : true;
            return matchesRegion && matchesSearch;
        });
    }, [country, search, activeRegion]);

    function handleRegionClick(region: string) {
        const next = activeRegion === region ? "" : region;
        setActiveRegion(next);
        router.push(next ? `/countries?region=${next}` : "/countries");
    }

    return (
        <div>
            {/* search + filter bar */}
            <div className="flex flex-col md:flex-row gap-4 mb-10">
                <input type="text" placeholder="Search countries..." value={search} onChange={(e) => setSearch(e.target.value)}
                    className="flex-1 gb-white/5 border border-white/10 rounded-full px-6 py-3 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-orange-400 transition-colors" />
            </div>

            <div className="flex gap-2 flex-wrap">
                {REGIONS.map((region) => (
                    <button key={region} onClick={() => handleRegionClick(region)}
                        className={`px-4 py-2 rounded-full text-xs tracking-wide border transition-colors ${activeRegion === region ?
                            "bg-green-400 text-black border-green-400 font-semibold"
                            : "border-white/10 text-white/60 hover:border-green-400 hover:text-green-400"
                            }`}>
                        {region}
                    </button>
                ))}
            </div>
        </div>

    )
}
